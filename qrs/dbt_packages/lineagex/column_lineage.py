...
try:
    from utils import dbt_find_column as _find_column
except ImportError:
    from .utils import dbt_find_column as _find_column
...
class ColumnLineage:
    def __init__(
        ...
        # 使用传入的 columns 参数
        self._resolve_column_dict(columns if columns else [])
    ...
    def _traverse_plan(self, plan: dict = None) -> None:
        ...
        # if scan from cte
            ...
            if "Alias" in plan.keys():
                all_cte_cols = [plan["Alias"] + "." + s for s in list(self.cte_dict[plan['CTE Name']].keys())]
            else:
                all_cte_cols = [plan["CTE Name"] + "." + s for s in list(self.cte_dict[plan['CTE Name']].keys())]
            self.possible_columns.extend(list(set(all_cte_cols).union(set(plan["Output"]))))
    ...
    def _add_cte_dict(self, plan: dict = None) -> None:
        ...
        if s[0] == "CTE":
            ...
            for idx, val in enumerate(self.cte_column[self.cte_name]):
                ...
                if plan["Output"][idx].find('SubPlan ') != -1:
                    subplan_name = re.findall(re.compile(r"SubPlan [0-9]*"), plan["Output"][idx])[0]
                    all_cols.extend(self.subplan_dict[subplan_name])
                temp_dict[val] = self._remove_table_alias(all_cols)
            self.cte_dict[self.cte_name] = temp_dict
            self.all_used_col = []
        ...
        # temporary subplan name
            ...
            for _, val in enumerate(plan['Output']):
                table_col = re.split(self.split_regex, val.strip())
                all_cols = list(
                    set(set(table_col) & set(self.possible_columns))
                )
    ...
    def _add_possible_columns(self, plan: dict = None) -> None:
        ...
        if invalid_list:
            all_table_cols = _find_column(
                table_name=plan["Schema"] + "." + plan["Relation Name"], engine=self.conn
            )
                ...
                if temp[0] in self.table_alias.keys():
                    ...
                    if org_name in self.cte_dict.keys():
                        ...
                        if temp[1] in temp_keys_dict.keys():
                            ret_cols.extend(self.cte_dict[org_name][temp_keys_dict[temp[1]]])
            ...
            # Resolve * and check if it is from a previous CTE or check database for the table
                ...
                if projection.find(exp.Identifier):
                    ...
                    if table_alias_dict[t_name] in cte_col_dict.keys():
                        ...
                        col_name = _find_column(table_name=table_alias_dict[t_name], engine=self.conn)
                # if * has no prefix
                else:
                    for t_name in cte_table_list:
                        if t_name in cte_col_dict.keys():
                            ...
                            cte_col_dict[cte_name].extend(_find_column(table_name=t_name, engine=self.conn))
