...
ON {{ schema }}.{{ table }} ({{ key }})
WHERE dbt_valid_to = '9999-12-31' AND dbt_is_deleted = 'False';
{% endset %}
...
ON {{ schema }}.{{ table }} (dbt_valid_from DESC, dbt_valid_to DESC);
{% endset %}
...
ON {{ schema }}.{{ table }} ({{ key }}, dbt_valid_from DESC);
{% endset %}
...
ON {{ schema }}.{{ table }} ({{ key }}, dbt_valid_from)
