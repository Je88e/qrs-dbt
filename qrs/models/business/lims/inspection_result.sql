 ...
 * 检验结果业务模型
 * 数据来源: LIMS系统
 * 业务描述: 记录检验结果详细信息，用于质量分析
 */

with inspection_result as (
    select * from {{ ref('lims_inspection_result') }}
),

inspection_task as (
    select * from {{ ref('lims_inspection_task') }}
),

sample as (
    select * from {{ ref('lims_sample') }}
),

test_item as (
    select * from {{ ref('lims_test_item') }}
)
