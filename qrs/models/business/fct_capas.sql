...
/*
    Model: fct_capas
    Description: 提供纠正预防措施(CAPA)的完整信息
    Enhancement: 添加投诉来源关联
*/
...
select
    ...
    case
        ...
        when c.source_type = '投诉' then cpl.complaint_code
        else null
    end as source_code,

    -- 投诉相关字段 (新增)
    case
        when c.source_type = '投诉' then cpl.complaint_type
        else null
    end as complaint_type,
    case
        when c.source_type = '投诉' then cpl.complaint_source
        else null
...
left join complaint cpl on c.source_id = cpl.complaint_id and c.source_type = '投诉'
